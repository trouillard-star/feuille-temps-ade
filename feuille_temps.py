...
║         © 2025 Thierry Rouillard — Tous droits réservés      ║
║         Développé par Thierry Rouillard pour Groupe ADE      ║
╚══════════════════════════════════════════════════════════════╝
"""

import sys, os, json, zipfile, threading, tempfile, shutil, subprocess
...
# ── Auto-update ───────────────────────────────────────────────────────────────
# ⚠️  Remplace GITHUB_USER et GITHUB_REPO par les tiens avant de builder
...
def _version_tuple(v: str):
    """'v3.4' → (3, 4)  — tolère préfixe v/V et chiffres manquants."""
    v = v.strip().lstrip("vV")
    parts = []
    for p in v.split("."):
...
# ── Thread de vérification de mise à jour ────────────────────────────────────
class UpdateSignals(QObject):
    update_available = pyqtSignal(str, str)   # (version_remote, notes)
    check_failed     = pyqtSignal()

class UpdateChecker(threading.Thread):
    """Vérifie silencieusement GitHub au démarrage (thread daemon)."""
...
# ── Téléchargement + remplacement de l'exe ───────────────────────────────────
class UpdateDownloader(threading.Thread):
    """Télécharge le nouvel exe, écrit un script .bat qui remplace l'ancien et relance."""
    def __init__(self, signals: UpdateSignals, progress_cb, done_cb, error_cb):
        super().__init__(daemon=True)
        self.signals      = signals
        self.progress_cb  = progress_cb   # appelé avec (bytes_dl, total_bytes)
...
def section_lbl(text):
    l = QLabel(text)
    l.setStyleSheet(f"""
        color: {TXT3}; font-size: 9px; font-weight: 700;
        letter-spacing: 1.5px; background: transparent; padding-bottom: 2px;
    """)
    return l
...
class LogoWidget(QFrame):
    def __init__(self):
        ...
        badge.setStyleSheet(f"""
            ...
            font-family: 'Segoe UI'; font-size: 13px; font-weight: 800; letter-spacing: 1px;
        """)
        glow(badge, ACC, 16)
        lay.addWidget(badge)
        ...
        tc.addWidget(name); tc.addWidget(sub)
...
class AutoCompleteRegistry:
            ...
            try:
                ...
                for j in JOURS:
                    for r in d.get("jours", {}).get(j, []):
                        p = r.get("projet","").strip(); de = r.get("desc","").strip()
                        if p: cls._projets.add(p)
                        if de: cls._descs.add(de)
            except: pass

    @classmethod
    def add(cls, projet, desc):
    ...
    def suggest(cls, text, kind):
        if not text: return ""
        pool = cls._projets if kind == "projet" else cls._descs
        tl = text.lower()
...
class AutoEntry(QLineEdit):
    def __init__(self, kind, placeholder="", width=None, height=32):
        super().__init__()
        self._kind = kind; self._suggestion = ""; self._accepting = False
    ...
    def keyPressEvent(self, event):
        if self._suggestion and event.key() in (Qt.Key.Key_Tab, Qt.Key.Key_Return):
            ...
            self._suggestion = ""; self._accepting = False
            event.accept(); return
...
class RowWidget(QWidget):
    ...
    def __init__(self, jour, row_idx, parent_day):
        ...
        self.date_lbl = lbl("", 10, color=TXT3); self.date_lbl.setFixedWidth(108)
        lay.addWidget(self.date_lbl)

        self.start  = styled_entry("08:00", 78)
        self.end    = styled_entry("16:30", 78)
        self.projet = AutoEntry("projet", "Projet...", 185)
        self.desc   = AutoEntry("desc",   "Description...")
        self.desc.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)

        self.dur_lbl = QLabel("–")
        self.dur_lbl.setFixedWidth(70)
        self.dur_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.dur_lbl.setStyleSheet(_DUR_OFF)
        ...
        self.del_btn.setStyleSheet(f"""
            QPushButton {{
                background:{CARD2}; color:{TXT3};
                border:1px solid {BORDER}; border-radius:6px; font-size:10px; font-weight:600;
            }}
            QPushButton:hover {{ background:{RED}; color:white; border-color:{RED}; }}
        """)

        for w in (self.start, self.end, self.projet, self.desc, self.dur_lbl, self.del_btn):
            lay.addWidget(w)
    ...
    def _reg(self): AutoCompleteRegistry.add(self.projet.text(), self.desc.text())

    def set_date(self, d): self.date_lbl.setText(d.strftime("%Y-%m-%d"))
    def get_data(self):    return {"debut":self.start.text(),"fin":self.end.text(),"projet":self.projet.text(),"desc":self.desc.text()}
    def total_h(self):     return calc_h(self.start.text(), self.end.text())
    def has_data(self):    return any(f.text().strip() for f in (self.start, self.end, self.projet, self.desc))
    ...
    def set_data(self, data):
        ...
        self.projet.setText(data.get("projet","")); self.desc.setText(data.get("desc",""))
        self._on_time()

    def clear(self):
        for field in (self.start, self.end):
            field.blockSignals(True); field.clear(); field.blockSignals(False)
        self.projet.clear(); self.desc.clear()
        self.dur_lbl.setText("–"); self.dur_lbl.setStyleSheet(_DUR_OFF)

# ── DayCard ───────────────────────────────────────────────────────────────────
_COL_HDR = [("Date",108),("Début",78),("Fin",78),("Projet",185),("Description",0),("Durée",70),("",32)]
...
class DayCard(QFrame):
    ...
    def __init__(self, jour, idx):
        ...
        self.date_lbl.setStyleSheet(f"color:{TXT2}; font-size:11px; background:transparent;")
        ...
        self.tot_lbl = QLabel("–")
        self.tot_lbl.setStyleSheet(f"color:{ACC2}; font-size:12px; font-weight:700; background:transparent;")
        tbl.addWidget(self.tot_lbl)
        hl.addWidget(self.tot_badge); hl.addSpacing(6)
        ...
        cl = QHBoxLayout(ch); cl.setContentsMargins(10,0,8,0); cl.setSpacing(6)
        for txt, w in _COL_HDR:
            l = QLabel(txt)
            l.setStyleSheet(f"color:{TXT3}; font-size:9px; font-weight:700; letter-spacing:1px; background:transparent;")
    ...
    def load_data(self, rows_data):
        for d in rows_data:
            if any(d.get(k,"").strip() for k in ("debut","fin","projet","desc")): self.add_row(data=d)
...
class HistoriqueDialog(QDialog):
    ...
    def __init__(self, parent):
        ...
        self.setWindowTitle("Historique — Groupe ADE")
        ...
        sl.addWidget(self.search); lay.addWidget(sf)
        ...
        scroll.setWidget(self.list_widget); lay.addWidget(scroll)
        ...
        fl.addWidget(lbl(f"{DEV_CREDIT}  ·  Groupe ADE", 9, color=TXT3))
        lay.addWidget(foot)
        self._refresh()
    ...
    def _refresh(self):
        ...
        for monday in weeks:
            ...
            if tot>=35:    bar_c, bg_c = GREEN, "#091A10"
            elif tot>0:    bar_c, bg_c = AMBER, "#1A1408"
            else:          bar_c, bg_c = MUTED, CARD
            card = QFrame()
            card.setStyleSheet(f"background:{bg_c}; border:1px solid {BORDER}; border-radius:10px;")
            card.setFixedHeight(58)
            cl = QHBoxLayout(card); cl.setContentsMargins(0,0,12,0); cl.setSpacing(0)
            bar = QFrame(); bar.setFixedWidth(4)
            bar.setStyleSheet(f"background:{bar_c}; border-radius:2px; margin:8px 0;")
            cl.addWidget(bar)
            ...
            il.addWidget(lbl(f"{fmt_h(tot)} travaillées" if tot>0 else "Vide", 10, color=TXT2))
            cl.addWidget(inf, 1)
            btn = QPushButton("Ouvrir →"); btn.setFixedSize(80,26)
...
class ParamDialog(QDialog):
    ...
    def __init__(self, parent, settings):
        super().__init__(parent)
        self.s = settings
        self.setWindowTitle("Paramètres — Groupe ADE"); self.setFixedSize(400, 250)
        self.setStyleSheet(SS_BASE + f"QDialog {{ background:{BG}; }}")
        lay = QVBoxLayout(self); lay.setContentsMargins(0,0,0,0); lay.setSpacing(0)
        ...
        frm = QFrame(); frm.setStyleSheet(f"background:{CARD}; border:1px solid {BORDER}; border-radius:12px;")
        fl = QVBoxLayout(frm); fl.setContentsMargins(14,12,14,14); fl.setSpacing(6)
        fl.addWidget(lbl("Nom de l'employé", 10, bold=True, color=TXT2))
        self.emp = styled_entry("Votre nom complet", height=36)
        self.emp.setText(settings.get("employer",""))
        fl.addWidget(self.emp); bl.addWidget(frm)

        save_btn = QPushButton("💾  Enregistrer les paramètres"); save_btn.setFixedHeight(40)
        ...
        fl2.addWidget(lbl(f"{DEV_CREDIT}  ·  {COPYRIGHT}", 9, color=TXT3))
        lay.addWidget(foot)

    def _save(self): self.s["employer"] = self.emp.text(); self.saved.emit(self.s); self.accept()
...
class StatsDialog(QDialog):
    ...
    def _build(self):
        ...
        if self.proj_totals:
            ...
            ph = QHBoxLayout()
            ph.addWidget(lbl("Heures par projet",12,bold=True)); ph.addStretch()
            ph.addWidget(lbl(f"{len(self.proj_totals)} projet(s)",10,color=TXT3))
            pl.addLayout(ph)
            ...
            for idx,(proj,h) in enumerate(sorted(self.proj_totals.items(),key=lambda x:x[1],reverse=True)):
                ...
                bl2 = QFrame(); bl2.setFixedWidth(4); bl2.setStyleSheet(f"background:{color}; border-radius:2px; margin:4px 0;")
                rl.addWidget(bl2)
                info = QWidget(); info.setStyleSheet("background:transparent;")
                il = QVBoxLayout(info); il.setContentsMargins(12,0,0,0); il.setSpacing(4)
                tr = QHBoxLayout()
                tr.addWidget(lbl(proj,11,bold=True)); tr.addStretch()
                tr.addWidget(lbl(f"{fmt_h(h)}  ·  {h/all_h*100:.1f}%",11,color=color))
                il.addLayout(tr)
        ...
        for i,wk in enumerate(reversed(self.weeks)):
            ...
            for j in JOURS:
                v=wk["jours"].get(j,0)
                col = GREEN if v>=7 else TXT2 if v>0 else MUTED
        ...
        cl.addWidget(tc2); cl.addStretch()
        self._footer(lay)
    ...
    def _footer(self, lay):
        ...
        fl.addWidget(lbl(f"{DEV_CREDIT}  ·  {COPYRIGHT}",9,color=TXT3))
        lay.addWidget(foot)
...
class StatusBar(QFrame):
    def __init__(self):
        ...
        lay.addWidget(lbl(f"{DEV_CREDIT}",9,color=TXT3))
        lay.addWidget(lbl("  ·  ",9,color=TXT3))
        lay.addWidget(lbl(f"{COPYRIGHT}",9,color=TXT3))
        lay.addWidget(lbl("  ·  ",9,color=TXT3))
        lay.addWidget(lbl(f"Groupe ADE  {APP_VERSION}",9,bold=True,color=TXT3))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.resize(1460, 840); self.setMinimumSize(1100, 660)
        ...
        # Vérification mise à jour en arrière-plan (2s après démarrage)
    ...
    def _build(self):
        ...
        sep = QFrame(); sep.setFixedWidth(1); sep.setStyleSheet(f"background:{BORDER};")
        ml.addWidget(sep)
    ...
    def _build_sidebar(self):
        sb = QFrame(); sb.setFixedWidth(218)
        ...
        tfl.addWidget(self.total_big)
        tfl.addWidget(lbl("heures cette semaine",10,color=TXT2))
        ...
        ul1 = QHBoxLayout()
        ul1.addWidget(QLabel("🟢")); ul1.addSpacing(4)
        self.update_ver_lbl = lbl("Mise à jour disponible", 10, bold=True, color=GREEN)
        ul1.addWidget(self.update_ver_lbl); ul1.addStretch()
        ufl.addLayout(ul1)
        self.update_notes_lbl = lbl("", 9, color=TXT2)
        self.update_notes_lbl.setWordWrap(True)
        ufl.addWidget(self.update_notes_lbl)
        self.update_btn = QPushButton("⬇  Installer la mise à jour")
        self.update_btn.setFixedHeight(30)
        self.update_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.update_btn.setStyleSheet(f"""
            QPushButton {{
                background: qlineargradient(x1:0,y1:0,x2:1,y2:0,stop:0 {GREEN2},stop:1 {GREEN});
                color: #0A1A0A; border:none; border-radius:7px;
        ...
        ufl.addWidget(self.update_btn)
        self.update_prog = QProgressBar()
        self.update_prog.setFixedHeight(5); self.update_prog.setTextVisible(False)
        self.update_prog.setStyleSheet(f"""
            QProgressBar {{ background:{BG}; border-radius:2px; border:none; }}
            QProgressBar::chunk {{ background:{GREEN}; border-radius:2px; }}
        """)
        self.update_prog.hide()
        ufl.addWidget(self.update_prog)
        self.update_frame.hide()
        lay.addWidget(self.update_frame)
        lay.addSpacing(4)
        ...
        fl.addWidget(lbl(COPYRIGHT,8,color=TXT3))
        lay.addWidget(foot)
        return sb

    def _nav_btn(self, text):
        b = QPushButton(text); b.setFixedSize(28,28)
        b.setCursor(Qt.CursorShape.PointingHandCursor)
        b.setStyleSheet(f"""
    ...
    def _build_topbar(self):
        ...
        self.emp_entry.textChanged.connect(self._mark_dirty)
        tl.addWidget(self.emp_entry)

        self.week_info_lbl = lbl("", 11, color=TXT2)
        tl.addWidget(self.week_info_lbl); tl.addStretch()

        self.week_total_lbl = QLabel("Total : 0h")
        self.week_total_lbl.setStyleSheet(f"""
            color:{ACC2}; font-size:14px; font-weight:700;
            background:{ACCD}; border:1px solid {ACC3}; border-radius:9px; padding:3px 14px;
        """)
        tl.addWidget(self.week_total_lbl)

        self.save_btn = QPushButton("💾  Sauvegarder")
        self.save_btn.setFixedSize(148,36)
        self.save_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.save_btn.setStyleSheet(_SAVE_SS)
    ...
    # ── Navigation ────────────────────────────────────────────────────────────
    def _prev(self):   self._save(silent=True); self.monday -= timedelta(weeks=1); self._switch()
    def _next(self):   self._save(silent=True); self.monday += timedelta(weeks=1); self._switch()
    def _today(self):  self._save(silent=True); self.monday = get_monday(self.today); self._switch()
    def _switch(self):
    ...
    def _load_week(self, monday):
        ...
        self.week_lbl.setText(lbl_txt)
        self.week_info_lbl.setText(f"Semaine du {lbl_txt}")
            ...
            try:
                ...
                for j in JOURS:
                    rows = [r for r in d.get("jours",{}).get(j,[])
                            if any(r.get(k,"").strip() for k in ("debut","fin","projet","desc"))]
                    self.day_cards[j].load_data(rows)
                    for r in rows: AutoCompleteRegistry.add(r.get("projet",""), r.get("desc",""))
                loaded = True
            except: pass
    ...
    def _mark_dirty(self, _=None):
        self._dirty = True; self.status_bar.set("● Non sauvegardé", AMBER)
    ...
    def _load_from_hist(self, monday):
        self._save(silent=True); self.monday = monday
        for j in JOURS: self.day_cards[j].clear_all()
    ...
    def _export_zip(self):
        ...
        dest, _ = QFileDialog.getSaveFileName(self, "Exporter — Groupe ADE",
            os.path.join(os.path.expanduser("~"), "Desktop",
                         f"GroupeADE_FeuilleTemps_{datetime.now().strftime('%Y-%m-%d')}.zip"), "ZIP (*.zip)")
        if not dest: return
        try:
            count = 0
            ...
            QMessageBox.information(self,"Export réussi — Groupe ADE",f"✓ {count} fichier(s) exporté(s)\n\n{dest}\n\n{COPYRIGHT}")
    ...
    def _import_zip(self):
        ...
        try:
            with zipfile.ZipFile(src,"r") as zf:
                ...
                rep = QMessageBox.question(self,"Confirmer l'import",
                    f"{len(jsons)} semaine(s)\n• {new} nouvelle(s)\n• {len(jsons)-new} à écraser\n\nContinuer ?")
    ...
    def _start_update_check(self):
        ...
        # check_failed: silencieux, on ne dérange pas l'utilisateur
        checker = UpdateChecker(self._upd_signals)
        checker.start()

    def _on_update_found(self, remote_version: str, notes: str):
        """Appelé depuis le thread Qt — affiche le bouton dans la sidebar."""
        ...
        self.status_bar.set(f"🟢 Mise à jour {remote_version} disponible", GREEN)

    def _do_update(self):
        self.update_btn.setEnabled(False)
        self.update_btn.setText("Téléchargement…")
        self.update_prog.setValue(0)
        self.update_prog.show()
        self._save(silent=True)

        def on_progress(dl, total):
            if total > 0:
                pct = int(dl / total * 100)
                # Thread-safe via QTimer
                QTimer.singleShot(0, lambda: self.update_prog.setValue(pct))

        def on_done():
            QTimer.singleShot(0, self._update_ready)

        def on_error(msg):
            QTimer.singleShot(0, lambda: self._update_error(msg))

        dl = UpdateDownloader(self._upd_signals, on_progress, on_done, on_error)
        dl.start()

    def _update_ready(self):
        self.update_btn.setText("✓ Redémarrage…")
        self.update_prog.setValue(100)
        QMessageBox.information(self, "Mise à jour — Groupe ADE",
    ...
    def _update_error(self, msg: str):
        ...
        self.update_btn.setText("⬇  Réessayer")
        self.update_prog.hide()
        QMessageBox.warning(self, "Erreur de mise à jour",
    ...
    def _pdf(self):
        ...
        for jour in JOURS:
            ...
            for txt, x in [("DATE",28),("DÉBUT",124),("FIN",178),("PROJET",232),("DESCRIPTION",380),("DURÉE",PW-58)]:
            ...
            for i,r in enumerate(rows):
                ...
                c.drawString(28,y-7,r.date_lbl.text()); c.drawString(124,y-7,r.start.text())
                c.drawString(178,y-7,r.end.text()); c.drawString(232,y-7,r.projet.text()[:22])
                c.drawString(380,y-7,r.desc.text()[:25])
                c.setFillColorRGB(0.165,0.369,0.969)
        ...
        c.save()
        rep = QMessageBox.question(self,"PDF créé — Groupe ADE",f"✓ PDF exporté !\n\n{dest}\n\nOuvrir le fichier ?")
